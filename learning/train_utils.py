...
def loss_batch(model, loss_func, xb, yb, opt=None):
    loss = loss_func(
        model(xb), torch.argmax(yb, dim=1)
...
def fit(
    ...
    for epoch in range(epochs):
        ...
        model.train()
        train_bar = tqdm(
            train_dl, total=len(train_dl), desc="Epoch {:d}/{:d}".format(epoch, epochs)
        )
...
class CheckpointSaver:
    ...
    def save(self, current_epoch, current_val_loss):
        if self.best_val_loss is None:
            ...
            print(
                "New best achieved at epoch {}: val_loss={}".format(
                    current_epoch, current_val_loss
                )
            )
            self.best_val_loss = current_val_loss
        else:
